...
/***************************
    Box making functions
***************************/

void box(Scene &scene, int h, int w, int d, RGB cceil, RGB cfloor, RGB cright, RGB cleft, RGB cback, bool zforward = true) {
    // ceil
...
void box(Scene &scene, int h, int w, int d, RGB cceil, RGB cfloor, RGB cright, RGB cleft, RGB cback, int lw, int ld, RGB lpower, int lz, bool zforward = true) {
    box(scene, h, w, d, cceil, cfloor, cright, cleft, cback, zforward);
...
void escena_conejos(int width, int height, std::shared_ptr<Camera> &c, Scene &scene) {

    // c = Camera(orig, left, up, front);
    // c = Camera(Point3(0, 0, -10), Vector3(3, 0, 0), Vector3(0, 3, 0), Vector3(0, 0, 10));
    c = std::make_shared<PinholeCamera>(35, Point3(0, -2, 0), 20, (float)width / height);
    ...
    box(scene, h, w, d, white, white, green, red, white);
...
void escena_dragon(int width, int height, std::shared_ptr<Camera> &c, Scene &scene) {

    c = std::make_shared<PinholeCamera>(30, Point3(0, 0, 0), 15, (float)width / height);
    ...
    box(scene, 3, 3, 3, white, white, green, red, blue, 2, 2, power, 0);
...
void escena_dof(int width, int height, std::shared_ptr<Camera> &c, Scene &scene) {

    c = std::make_shared<ThinLensCamera>(25, Point3(0, 0, -1), 20, (float)width / height, 4);
...
void dielectric(int width, int height, std::shared_ptr<Camera> &c, Scene &scene) {
...
void texture_1(int width, int height, std::shared_ptr<Camera> &c, Scene &scene) {
...
void texture_2(int width, int height, std::shared_ptr<Camera> &c, Scene &scene) {
...
void spheres(int width, int height, std::shared_ptr<Camera> &c, Scene &scene) {
    // c = std::make_shared<PinholeCamera>(30.0f, Point3(50, 52, 295.6), 0.1f, (float)width / (float)height);
    c = std::make_shared<PinholeCamera>(30.0f, Point3(50, 52, 295.6), -5.0f, (float)width / (float)height);
    scene.set_background(RGB(0, 0, 0));

    // float scale = 1e5;
    float scale = 1e4;

    // NOTA: Hace falta cambiar EPSILON a 0.01 en shape.hpp,
    // las esferas tienen que ser demasiado grandes, y al usar floats pierde precisión
    ...
    // Poner la esfera luminosa un poco más abajo cuando se usa scale < 1e5
...
void textures(int width, int height, std::shared_ptr<Camera> &c, Scene &scene) {
...
void sudor(int width, int height, std::shared_ptr<Camera> &c, Scene &scene) {

    Image texture = readPPM("../textures/sudor.ppm");
    Image texture2 = readPPM("../textures/world.ppm");
    Image texture3 = readPPM("../textures/ray_tracer.ppm");

    c = std::make_shared<PinholeCamera>(50.0f, Point3(0, -2, 5), 22.0f, (float)width / (float)height);
    ...
    TriangleMesh tm = TriangleMesh(triangle_mesh, std::make_shared<Dielectric>(1.52f), Point3(-5, -4.5, -3), 25, true);
    scene.add_shape(std::make_shared<TriangleMesh>(tm));
    TriangleMesh tm2 = TriangleMesh(triangle_mesh, std::make_shared<PerfectSpecular>(0.95f), Point3(5.5, -4.5, -3), 25, true);
    scene.add_shape(std::make_shared<TriangleMesh>(tm2));
...
void escena_concurso(int width, int height, std::shared_ptr<Camera> &c, Scene &scene) {

    // c = std::make_shared<ThinLensCamera>(32, Point3(0, -1, -0.4), 25, (float)width / height, 1);
    c = std::make_shared<PinholeCamera>(32, Point3(0, -1, -0.4), 25, (float)width / height);
    ...
    // Area light
    box(scene, h, w, d, white, white, green, red, white, 5, 5, power, 0, true);
...
/*
void escena_test(int width, int height, std::shared_ptr<Camera> &c, Scene &scene) {
    // c = Camera(orig, left, up, front);
    // c = Camera(Point3(0, 0, -10), Vector3(3, 0, 0), Vector3(0, 3, 0), Vector3(0, 0, 10));
    c = std::make_shared<PinholeCamera>(30, Point3(0, 0, 0), 15, (float)width / height);
...
void prueba_geometria(int width, int height, std::shared_ptr<Camera> &c, Scene &scene) {
    width = 1920;
    height = 1080;
...
void escenaPhong(int &width, int &height, std::shared_ptr<Camera> &c, Scene &scene) {
...
void escenaDielectrico(int width, int height, std::shared_ptr<Camera> &c, Scene &scene) {
...
void escenaChula(int width, int height, std::shared_ptr<Camera> &c, Scene &scene) {
    ...
    c = std::make_shared<PinholeCamera>(30.0f, Point3(0, -2, 0), 19.0f, (float)width / (float)height);
...
void escenaConejo(int width, int height, std::shared_ptr<Camera> &c, Scene &scene) {
...
void escenaBVH(int width, int height, std::shared_ptr<Camera> &c, Scene &scene) {
...
void escenaDOF(int &width, int &height, std::shared_ptr<Camera> &c, Scene &scene) {
    c = std::make_shared<ThinLensCamera>(25, Point3(0, 0, 2), 20, (float)width / height, 2);
...
void cornellBox(std::shared_ptr<Camera> &c, Scene &scene) {

    c = std::make_shared<PinholeCamera>(Point3(278, 273, -800), Vector3(1, 0, 0), Vector3(0, 1, 0), Vector3(0, 0, 2));
...
void escenaMartins(std::shared_ptr<Camera> &c, std::vector<std::shared_ptr<Shape>> &scene) {
    Point3 o = Point3(2, 6, 0);
    Vector3 r = Vector3(0, 0, 1);
    Vector3 u = Vector3(0, 1, 0);
    Vector3 f = Vector3(1, 0, 0);
    c = std::make_shared<PinholeCamera>(o, r, u, f);
...
void escenaPLY(std::shared_ptr<Camera> &c, std::vector<std::shared_ptr<Shape>> &scene) {
    c = std::make_shared<PinholeCamera>(Point3(896, 595, -200), Vector3(2000, 0, 0), Vector3(0, 2000, 0), Vector3(0, 0, 200));
